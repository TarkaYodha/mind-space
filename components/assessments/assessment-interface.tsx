...
export function AssessmentInterface({ assessmentId }: { assessmentId: string }) {
  ...
  function generateRecommendations(score: number, severity?: string) {
      ...
      >
        ...
        <div className={`bg-${severityColor}-50 border border-${severityColor}-200 rounded-2xl p-6`}>
          ...
          <p className={`text-sm font-medium text-${severityColor}-800`}>
            Severity Level: <span className="capitalize">{result.severity.replace('_', ' ')}</span>
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Link href="/tools" className="bg-[#001f4d] hover:bg-[#001437] text-white px-6 py-3 rounded-lg font-semibold transition-colors text-center inline-block">
            View Resources
          </Link>
