...
export async function POST(request: NextRequest) {
  try {
    ...
    let usedService = 'openrouter'

    // Try OpenRouter first (primary service)
    if (OPENROUTER_API_KEY) {
      try {
        console.log('Using OpenRouter as primary service...')
        const response = await fetch(`${OPENROUTER_BASE_URL}/chat/completions`, {
          ...
          headers: {
            ...
            'X-Title': 'MindSpace Mental Health Assistant',
          },
          body: JSON.stringify({
            model: 'openai/gpt-4o-mini', // Free GPT-4o mini model - WORKING ✅
            messages: [
              {
                role: 'system',
        ...
        console.warn('OpenRouter failed, falling back to OpenAI:', error)
        usedService = 'openai'
      }
    } else if (!aiResponse) {
      usedService = 'openai'
